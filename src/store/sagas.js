...
function* jsonplaceholder_post_posts_readWorker(action) {
  try {
    const result = yield call(
      apiService.jsonplaceholder_post_posts_read,
      action
    )
    yield put(actions.jsonplaceholder_post_posts_readSucceeded(result, action))
  } catch (err) {
    yield put(actions.jsonplaceholder_post_posts_readFailed(err, action))
  }
}
function* jsonplaceholder_post_posts_readWatcher() {
  yield takeEvery(
    types.JSONPLACEHOLDER_POST_POSTS_READ,
    jsonplaceholder_post_posts_readWorker
  )
}
function* jsonplaceholder_delete_posts_readWorker(action) {
  try {
    const result = yield call(
      apiService.jsonplaceholder_delete_posts_read,
      action
    )
    yield put(
      actions.jsonplaceholder_delete_posts_readSucceeded(result, action)
    )
  } catch (err) {
    yield put(actions.jsonplaceholder_delete_posts_readFailed(err, action))
  }
}
function* jsonplaceholder_delete_posts_readWatcher() {
  yield takeEvery(
    types.JSONPLACEHOLDER_DELETE_POSTS_READ,
    jsonplaceholder_delete_posts_readWorker
  )
}
...
export default function* rootSaga() {
  const sagas = [
    jsonplaceholder_post_posts_readWatcher,
    jsonplaceholder_delete_posts_readWatcher,
